...
RMF::RMF(const BSCurve &c, const Vector3D &start, const Vector3D &end)
  : curve_(c) {
  ...
  VectorVector der; c.eval(0.0, 1, der);
  Frame f(0.0, 0.0, der[0], der[1].normalize(), start);
  ...
  angleCorrection_ = std::acos(std::min(std::max(end * rmfEnd, -1.0), 1.0));
  if (((rmfEnd - end) ^ end) * f.d < 0.0)
...
RMF::Frame
RMF::nextFrame(const BSCurve &c, const Frame &prev, double u) {
  VectorVector der; c.eval(u, 1, der);
  der[1].normalize();
  ...
  return Frame(u, prev.s + c.arcLength(prev.u, u), der[0], der[1], nNext);
}
